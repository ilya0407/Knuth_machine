...
void bz(int oper1, int oper2, int oper3)
{
	if(registers_general[oper1].octabyte == 0)
	{
		current_addr += (short int)oper3 * 4;
	}
}
...
struct operation operations[OP_COUNT] = {
	...
	{bdif,0xD0},{bdif,0xD1},{wdif,0xD2},{wdif,0xD3},{tdif,0xD4},{tdif,0xD5},{odif,0xD6},{odif,0xD7},
...
void get_code(int argc,char** argv)
{
	...
	{
		fgets(str,5,f);
		addr_in_mem = str_to_int(str);
		fgets(str,5,f);
		start_addr = str_to_int(str);
		fgets(str,5,f);
		num_loadbytes = str_to_int(str);
	}
	int j = 0;
	int num_bytes = 0;
	while(1)
	{
		...
		fgets(op0,3,f);
		if(feof(f)) break;
		if(op3 != NULL && op2 != NULL && op1 != NULL && op0 != NULL)
		{
			info = str_to_int(op3);
			virtual_memory[(num_bytes + addr_in_mem)/8].byte[3+j*4] = info;
			info = str_to_int(op2);
			virtual_memory[(num_bytes + addr_in_mem)/8].byte[2+j*4] = info;
			info = str_to_int(op1);
			virtual_memory[(num_bytes + addr_in_mem)/8].byte[1+j*4] = info;
			info = str_to_int(op0);
			virtual_memory[(num_bytes + addr_in_mem)/8].byte[0+j*4] = info;
...
typedef union{
int tetrabyte;
unsigned int utetrabyte;
char byte[4];
unsigned char ubyte[4];
}tetr;

void cmd(int flag_exit)
{
	tetr mas[num_loadbytes/4];
	
	for(int i = 0; i<=num_loadbytes/4; i+=2)
	{
		mas[i].tetrabyte = virtual_memory[(start_addr + i*4)/8].tetrabyte[0];
		mas[i+1].tetrabyte = virtual_memory[(start_addr + i*4)/8].tetrabyte[1];
	}
	int n = 0;
	...
	while(1)
	{
		if(cf == 1) i = (current_addr-start_addr)/4;
		if(cf == 0)
			current_addr += 4;
			
		cadr = current_addr;
		n = find_in_mas(mas[i].ubyte[3]);
		
		if(mas[i].ubyte[3]%2 == 1)
			flag_const = 1;
		else
			flag_const = 0;
		
		if(n!=-1)
			(*operations[n].opr)(mas[i].byte[2],mas[i].byte[1],mas[i].byte[0]);
		if(current_addr != cadr)
			cf = 1;
		else 
			cf = 0;
		if(n==0)
